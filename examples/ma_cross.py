...
def sma(rates_df, window):
    return rates_df.rolling(window=window).mean().values
...
class MACrossEA(ExpertAdvisor):
    ...
    def on_tick(self, tick: Tick):
        ...
        rates_h1 = self.GetNRatesByStartPosition(self.symbol, ENUM_TIMEFRAME.PERIOD_H1, 0, 10)
        rates_m15 = self.GetNRatesByStartPosition(self.symbol, ENUM_TIMEFRAME.PERIOD_M15, 0, 10)
        
        if rates_h1 is not None and rates_m15 is not None:
            sma_h1 = sma(rates_h1["close"], window=2)
            sma_m15 = sma(rates_m15["close"], window=5)
